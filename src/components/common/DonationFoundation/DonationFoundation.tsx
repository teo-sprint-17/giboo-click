...
const DonationFoundation = ({ foundation, size }: DonationFoundationType) => {
  ...
  return (
    ...
    >
      <div>
        <p
          className={`font-["SUITE Variable"] font-extrabold leading-29.95 text-left whitespace-pre-wrap ${
        ...
        <div
          className={`flex justify-between font-["SUITE Variable"] font-semibold ${
      ...
      <div className={`w-full bottom-[10px] ${size === 'sm' && 'h-[50px]'}`}>
        <div className="h-full bg-white bg-opacity-75 rounded-[10px] px-[10px] py-[14px] flex flex-col gap-[5px]">
          ...
          <div className="w-full flex justify-between">
            <p className={`font-['SUITE-Variable'] ${size === 'sm' ? ' text-[11px]' : 'text-[14px]'}`}>
              총 {donationCount}명이 참여중입니다.
            </p>
            <p className={`font-['SUITE-Variable'] ${size === 'sm' ? ' text-[10px]' : 'text-[12px]'}`}>
              {donationGoalPercentile}%
            </p>
          </div>
        </div>
      </div>
